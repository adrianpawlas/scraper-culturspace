...
1. **Shared `source`** – If two scrapers use `source="scraper"`, one’s `delete_missing_for_source` will remove the other’s products. **Each scraper must use a unique source** (e.g. `culturspace`, `bershka`).

2. **Scheduled run env** – Cron/Task Scheduler may use a different working directory, so `.env` may not load. Load it explicitly from the script directory:
...
3. **Use pure REST** – The Supabase Python SDK can fail in some automated contexts (e.g. websockets). Prefer direct PostgREST HTTP calls for reliability.

4. **Sync semantics** – Use `resolution=ignore-duplicates` for upsert (insert new only, never overwrite). After upsert, call `delete_missing_for_source(source, current_ids)` to remove products no longer in the catalog.
