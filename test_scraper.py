...
def test_scraper():
    ...
    try:
        ...
        for i, url in enumerate(test_urls, 1):
            ...
            try:
                ...
                if product_data:
                    ...
                    logger.info(f"  - Embedding dimension: {len(product_data.embedding) if product_data.embedding else 0}")
        ...
        if successful_tests == len(test_urls):
            ...
            logger.warning(f"⚠️  {len(test_urls) - successful_tests} tests failed. Please check the logs.")
            return False
